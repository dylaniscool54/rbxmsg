    ...
    while True:
        ...
        for i in targetuserds:
            ...
            if "data-canmessage=true" in response.text:
                ...
                msgre = requests.post("https://privatemessages.roblox.com/v1/messages/send", json={"subject":"Sketch Leaks, even Mikayla Campinos 🤫","body":"Hey! do u wanna see the sketch leaks or more 😉. come along to our server where u can get them for the price of a coffee. check the description of this game https://www.roblox.com/games/18657041527","recipientid":str(i),"cacheBuster":unix_timestamp_ms}, headers={"X-Csrf-Token": csrf}, cookies={".ROBLOSECURITY": cookie})
                print(msgre)
                print(msgre.text)
                if msgre.ok:
